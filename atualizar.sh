#!/usr/bin/env bash
# Sobe a versao do plugin harteg-studio e publica no GitHub.
# Uso:
...
PLUGIN_JSON="plugins/harteg-studio/.claude-plugin/plugin.json"
...
if git diff --cached --quiet; then
  echo "Nada para fazer commit."; exit 0
fi
git commit -m "harteg-studio v$NEW"
echo "A enviar para o GitHub..."
git push
echo ""
echo "OK! Publicado v$NEW."
echo "Quem ja instalou recebe ao atualizar o marketplace:"
