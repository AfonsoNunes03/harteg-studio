# 05 — slides.json e entrega no Harteg Studio

A entrega é o editor **Harteg Studio** (`assets/carousel-studio.html`) já populado.
Montas um `slides.json` e **injeta-lo** no editor (não há motor de build).
...
```
Apresenta o `carrossel-editor.html`. Tudo (edição, tipos de letra, cores, posicionamento, imagem de fundo, cantos, logótipo e **Exportar ZIP**) acontece dentro dele. NÃO uses `build_carrossel.py` nem `editor_template.html` — foram removidos.
...
- `tweet`: card de post/rede social.
  ...
  { "tipo":"tweet", "titulo":"Headline opcional", "sub":"linha opcional",
    ...
    "texto":"Texto do post. Usa **palavra** para realçar.",
